	...
	public function upload ($lang='english')
	{
		...
		if ($this->upload->do_upload()) // Success
		{
			...
			//$result['file_name']	= $conf['img_path'] . '/' . $result['file_name'];
						//$result['file_name']	= 'http://www.notariaordono16.es/images/blog' . '/' . $result['file_name'];
			            //$result['file_name']	= $_SERVER['DOCUMENT_ROOT'] .  $conf['img_path'] . '/' . $result['file_name'];
$result['file_name']	= 'http://notariaordono16.es/images/blog' . '/' . $result['file_name'];
